...
export default async function PatientInvitationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // 17/09/2026 — pedido do usuário: vir com uma frase amigável antes do
  // código, em vez de mandar só "OPTOTICA <token>" seco. O servidor
  // (app/api/webhooks/whatsapp/route.ts) foi ajustado para aceitar o código
  // em qualquer ponto da mensagem — a frase antes não quebra o reconhecimento.
  const message = encodeURIComponent(`Olá! Segue o código para ser atendido: OPTOTICA ${token}`);
