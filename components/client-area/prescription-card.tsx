...
export function PrescriptionCard({ issuedId, od, oe, observations }: {
  ...
  return <>
    ...
    <div className="rx-scroll"><table className="rx-table rx-table-compact"><thead><tr><th></th><th>Esférico</th><th>Cilíndrico</th><th>Eixo</th><th>Adição</th></tr></thead><tbody>
      <tr><th>OD</th><td>{formatSignedSphere(od.esferico) || '—'}</td><td>{formatDiopter(od.cilindrico) || '—'}</td><td>{formatAxis(od.eixo) || '—'}</td><td>{formatDiopter(od.adicao) || '—'}</td></tr>
      <tr><th>OE</th><td>{formatSignedSphere(oe.esferico) || '—'}</td><td>{formatDiopter(oe.cilindrico) || '—'}</td><td>{formatAxis(oe.eixo) || '—'}</td><td>{formatDiopter(oe.adicao) || '—'}</td></tr>
    </tbody></table></div>
