...
export default async function ClientPage() {
  ...
  const { data } = await supabase.from('orders').select('id, order_number, status').eq('client_id', client.id).order('order_number', { ascending: false });
  ...
  const current = orders.find((order) => order.status !== 'completed') || orders[0];
