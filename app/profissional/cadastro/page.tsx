...
export default async function ProfessionalRegistrationPage() {
  ...
  const initialValues = {
    ...
    councilRegistration: profile?.council_registration || '',
    technicalResponsibleName: profile?.technical_responsible_name || '',
    technicalResponsibleRegistration: profile?.technical_responsible_registration || '',
    cnpj: profile?.cnpj || '',
  ...
  return (
    <div className="page-shell form-shell">
      <section className="dashboard-head"><div><p className="eyebrow">Validação Optótica</p><h1>Dados profissionais</h1><p className="muted">Preencha os dados da atividade e dos laboratórios. Enquanto a análise estiver aberta, nenhum paciente fica disponível.</p></div></section>
