...
// 1) Sem `imageUrl` no corpo (comportamento original): usa
//    `source_image_url` da própria cor (migração 202609120004 + script de
//    dados 202609120005) — só funciona pra cor 'incompleto' (sem foto
//    nenhuma ainda).
// 2) Com `imageUrl` no corpo (13/09/2026, pedido do usuário: "Trocar foto"
//    buscando outra foto do AliExpress em vez de só aceitar upload manual):
//    usada pelo seletor de miniaturas da galeria geral do anúncio na tela —
//    funciona também pra cor 'pendente'/'rejeitada' (já tem foto, é uma
//    troca). A URL só é aceita se já estiver catalogada em
//    catalog_product_gallery_images pra este produto — nunca uma URL
//    arbitrária vinda do cliente, pra não expor o servidor a baixar
//    qualquer endereço (SSRF).
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Não foi possível baixar a foto (o servidor do fornecedor não respondeu). Tente de novo em instantes ou use "Adicionar foto".' }, { status: 502 });
  }
  if (!response.ok) {
    return NextResponse.json({ message: `Não foi possível baixar a foto (status ${response.status}). Use "Adicionar foto" para enviar manualmente.` }, { status: 502 });
  }
  ...
  if (buffer.byteLength === 0 || buffer.byteLength > MAX_BYTES) {
    return NextResponse.json({ message: 'A foto veio vazia ou grande demais. Use "Adicionar foto" para enviar manualmente.' }, { status: 502 });
  }
  ...
  const patch: Record<string, unknown> = { original_image_path: path, updated_at: now, status: 'pendente' };
  if (image.status === 'incompleto') {
