...
export async function POST(request: Request, { params }: { params: Promise<{ requestId: string }> }) {
  ...
  try {
    ...
    const { data: record } = await actor.admin.from('professional_verification_requests').select('id,professional_profile_id,status').eq('id', requestId).maybeSingle();
    ...
    if (decision === 'verified') {
      ...
      uploadedPath = `${record.professional_profile_id}/${record.id}/attestation-${randomUUID()}.pdf`;
    ...
    uploadedPath = null; // The database trigger records the decision and its previous evidence atomically.
    return NextResponse.json({ message: 'Decisão registrada.' });
  } catch (error) {
    if (uploadedPath) await actor.admin.storage.from(verificationBucket).remove([uploadedPath]);
