...
async function lookup(code: string) {
  try {
    ...
    const [{ data: profile }, { data: review }, { data: order }] = await Promise.all([
      ...
      admin.from('professional_verification_requests').select('status,professional_name,registration,course,institution,reviewed_at,valid_until,public_scope').eq('professional_profile_id', rx.professional_profile_id).order('created_at', { ascending: false }).limit(1).maybeSingle(),
...
export default async function VerifyPrescription({ params }: { params: Promise<{ code: string }> }) {
  ...
  const { rx, profile, review, order } = result;
    ...
    const verified = profile?.status === 'approved' && review?.status === 'verified' && review.valid_until >= today && review.professional_name === profile.display_name && review.registration === (profile.council_registration || '');
    ...
    return <div className="page-shell verification-shell">
      <section className="card verification-hero"><p className="eyebrow">Portal Optótica · Verificação</p><h1>Prescrição óptica</h1><span className="verification-status">{labels[status]}</span>
